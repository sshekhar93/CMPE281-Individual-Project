   ...
   // Node Sync Queues
    ...
    public static void startup() {
        try {
            ...
            Sync sync1 = new Sync( "10.0.1.14", node1_sync_queue ) ;
            new Thread(sync1).start() ;             
            Sync sync2 = new Sync( "10.0.1.146", node2_sync_queue ) ;
            new Thread(sync2).start() ;              
            Sync sync3 = new Sync( "10.0.1.154", node3_sync_queue ) ;
            new Thread(sync3).start() ;              
            Sync sync4 = new Sync( "10.0.1.30", node4_sync_queue ) ;
            new Thread(sync4).start() ;              
            Sync sync5 = new Sync( "10.0.1.131", node5_sync_queue ) ;
    ...
    public static void syncDocument( String key, String command ) {
        ...
        try {
            ...
            SyncRequest syncObject ;
            if(command.equals("delete"))
            {
                syncObject = API.get_sync_request( key, command ) ;
            }
            else
            {
    ...
    public int nodeIndex( String id ) {
        ...
        int index = 0 ;
        switch ( name ) {
            case "10.0.1.14": index = 1 ; break ;
            case "10.0.1.146": index = 2 ; break ;
            case "10.0.1.154": index = 3 ; break ;
            case "10.0.1.30": index = 4 ; break ;
            case "10.0.1.131": index = 5 ; break ;
            default: index = 0 ;
        }
